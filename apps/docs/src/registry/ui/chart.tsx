...
import { merge as mergeOptions } from "chart.js/helpers";
...
> = (props) => {
  const others = omit(props, "id", "class", "children", "config");
...
const BaseChart: Component<ChartProps> = (rawProps) => {
  ...
  const init = () => {
    ...
    const instance = new Chart(ctx, {
      type: props.type,
      data: resolveColors(props.data, element),
      options: resolveColors(props.options, element),
      plugins: props.plugins,
    });
    setChart(instance);
  };

  createEffect(canvasRef, (canvas) => {
    if (!canvas) {
      return;
    }
    untrack(init);
  ...
  createEffect(
    ...
    (data) => {
      untrack(() => {
        chart()!.data = resolveColors(data, resolveElement());
        chart()!.update();
      });
    },
    { defer: true },
  ...
  createEffect(
    ...
    (options) => {
      untrack(() => {
        chart()!.options = resolveColors(options, resolveElement());
        chart()!.update();
      });
    },
    { defer: true },
  ...
  createEffect(
    ...
    ([width, height]) => {
      untrack(() => chart()!.resize(width, height));
    },
    { defer: true },
  );
  ...
  createEffect(canvasRef, (canvas) => {
    ...
    const observer = new MutationObserver(() => {
      untrack(() => {
        ...
        const element = resolveElement();
        instance.data = resolveColors(props.data, element);
        instance.options = resolveColors(props.options, element);
        instance.update();
      });
    });
  ...
  return (
    <canvas
      ref={[props.ref, (el) => setCanvasRef(el)]}
      height={props.height}
      width={props.width}
      {...others}
    />
...
function createTypedChart(
  ...
  return (props) => (
    <BaseChart
      type={type}
      {...props}
      options={mergeOptions(defaultOptions(type, () => props.tooltip), [
        props.options ?? {},
      ])}
    />
  );
