...
function MobileLink(props: MobileLinkProps) {
  const others = omit(props, "class", "onOpenChange", "external");

  return (
    <a
      {...others}
      rel={props.external ? "external" : undefined}
      class={cn("flex items-center gap-2 font-medium text-2xl", props.class)}
