...
```bash
npx @ptzburn/shadcn-solidjs@beta add label
```
...
The `Label` component renders a `label` element and accepts every native
label attribute, including `for` to associate it with a control.

| Prop    | Type             | Default   |
| ------- | ---------------- | --------- |
| `as`    | `ValidComponent` | `"label"` |
| `class` | `string`         | -         |
