...
```bash
npx @ptzburn/shadcn-solidjs@beta add menubar
```
...
`MenubarLabel` must be placed inside a `MenubarGroup` or `MenubarRadioGroup`,
and `MenubarSubContent` portals itself, so it never needs a `MenubarPortal`
wrapper.
