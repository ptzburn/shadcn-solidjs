...
```bash
npx @ptzburn/shadcn-solidjs@latest add context-menu
```
...
Unlike the upstream radix port, `ContextMenuLabel` must be placed inside a
`ContextMenuGroup` or `ContextMenuRadioGroup`.
