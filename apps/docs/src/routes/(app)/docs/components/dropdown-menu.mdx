...
```bash
npx @ptzburn/shadcn-solidjs@beta add dropdown-menu
```
...
`DropdownMenuLabel` must be placed inside a `DropdownMenuGroup` or
`DropdownMenuRadioGroup`, and `DropdownMenuSubContent` portals itself, so it
does not need to be wrapped in a `DropdownMenuPortal`.
