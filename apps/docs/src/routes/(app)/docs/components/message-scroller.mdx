...
```bash
npx @ptzburn/shadcn-solidjs@beta add message-scroller
```
...
```bash
npm install @ptzburn/shadcn-solid@beta @kobalte/core@2.0.0-alpha.0
```
...
<Callout>
  The scroll engine ships in the `@ptzburn/shadcn-solid` package. The CLI
  installs it together with the styled parts in `message-scroller.tsx`.
</Callout>
...
<ComponentPreview name="message-scroller-anchoring" previewClassName="h-auto p-4 sm:p-10" />
...
<ComponentPreview name="message-scroller-group-chat" previewClassName="h-auto p-4 sm:p-10" />
...
<ComponentPreview name="message-scroller-previous-context" previewClassName="h-auto p-4 sm:p-10" />
...
<ComponentPreview name="message-scroller-streaming" previewClassName="h-auto p-4 sm:p-10" />
...
<ComponentPreview
  ...
  previewClassName="h-auto p-4 sm:p-10"
  hideCode
/>
...
<ComponentPreview name="message-scroller-load-history" previewClassName="h-auto p-4 sm:p-10" />
...
<Callout>
  Solid's `<For>` keys rows by object identity, so keep each message object
  stable while it updates: mutate a store in place (as the examples do) or pass
  `keyed={(message) => message.id}`. A row that is re-created counts as a new
  item — an anchored turn would re-anchor and an entrance animation would
  replay.
</Callout>

### Animating New Messages

`MessageScrollerItem` can be animated directly. A common chat pattern is to
animate the user's message when it is sent, then let the assistant reply stream
into a regular row below it. Keep `messageId` and `scrollAnchor` on the item and
add an entrance class — the docs use tw-animate-css presets, which only touch
...
<ComponentPreview name="message-scroller-animation" previewClassName="h-auto p-4 sm:p-10" />
...
Our performance goal for `MessageScroller` is to keep the scroll hot path outside of the reactive graph: no signal updates for transcript rows, no forced layout on every scroll, and as little off-screen paint work as the browser can avoid.
...
The behavior in `MessageScroller` comes from the `@ptzburn/shadcn-solid`
package, installed alongside the styled component. To use it directly with
your own markup and styles, import the namespace object instead of the flat
styled parts.
...
one subscriber is mounted. Call it from a component body (it subscribes in
`onSettled`), not from an event handler.
