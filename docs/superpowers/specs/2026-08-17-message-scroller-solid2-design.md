...
| unmount ref(null)                             | component `onCleanup` passing the instance's own element (`setXElement(null, node)`); the controller only nulls a ref that still points at that element — Solid 2 mounts a replacement branch before disposing the old one (Item unregisters the same way: `registerMessage(id, null, element)`) |
