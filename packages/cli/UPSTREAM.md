...
| `src/registry/constants.ts`            | `registry/constants.ts`                             | `{iconLibrary}` replaces upstream's `{style}` placeholder. Built-in namespace is `@shadcn-solidjs`. `REGISTRY_THEME_URL` is ours — see the note below.                                                                                                                                                                   |
